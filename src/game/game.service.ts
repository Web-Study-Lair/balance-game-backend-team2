...
export class GameService {
  ...
  async removeGame(deleteGameDto: DeleteGameDto) {
    const game = await this.gameRepository.findOneBy({ id: deleteGameDto.gameId });

    // gameId의 게임이 존재하지 않을 때의 예외처리
    if (game === null) {
      throw new NotFoundException(`gameId ${deleteGameDto.gameId} is not found`);
    }

    if (game.user_id !== deleteGameDto.user.userId) {
      throw new ForbiddenException(`userId ${deleteGameDto.user.userId} is not owner of gameId ${deleteGameDto.gameId}`);
    }
